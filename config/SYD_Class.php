					...
					<tbody>
						...
						while($row=$this->result->fetch_array(MYSQLI_NUM)){?>
							<tr>
								<?php 
								if($row[0]=="Total"){
									echo"<tr> <thead style=' background-color: #696cff !important; color: white;font-weight: 700;'>";
	...
	private function viewDataTable($qry1,$dtTble){
		...
		try{
			...
			if(!$condition){
				...
				<table id='<?php echo $dtTble; ?> example' class='table tables tbless table-striped table-bordered table-hover'>
					<thead class="thead bg-dark" style="color: white;">
						<tr>
							<?php 
							$lop=0;
							foreach($fields as $field){
								echo "<th style='color:white;'>$field->name</th>";
							}?>
						</tr>
					</thead>
					<tbody>
						...
						while($row=$this->result->fetch_array(MYSQLI_NUM)){ ?>
							<tr>
							<input class="inp" type="hidden" name="id" value="<?php echo $row[0]; ?>">
								<?php 
				...
				<table id='<?php echo $dtTble; ?>' class='table tables tbless table-striped table-bordered table-hover'>
					<thead class="thead bg-dark" style="color: red;">
						<tr>
							<?php 
							$lop=0;
							foreach($fields as $field){
						...
						while($row=$this->result->fetch_array(MYSQLI_NUM)){ ?>
							<tr>
								...
								<td class="td-actions text-center" style="padding: 10px;">
									...
									<button type="button" title='Edit' class="btn btn-primary btn-sm <?php echo "btn_edit_".str_replace('dt_', '', $dtTble); ?>" value="<?php echo $row[0];?>">

														<i class="ti-pencil"></i>update</button>
...
<button  type="button" title='Remove' class="btn btn-danger btn-sm <?php echo "btn_remove_".str_replace('dt_', '', $dtTble); ?>" value="<?php echo $row[0];?>">
	...
	public function fillCombo($qry,$id,$choose){
		$this->connection();
		echo"<div class='' id='test'> <select id='$id' class='form-control selectpicker input-rounded ' name='$id' style='width:100%;'  required='' data-live-search='true' title='Choose an option...'>";
		try{
			$this->result=$this->db->query($qry);
			$condition=(bool)mysqli_num_rows($this->result);
			echo $condition;
